...
class OrderKuota {
  ...
  async getTransactionQris(type = '') {
    ...
    return response && response.qris_history ? response : { qris_history: { results: [] } };
  }
  ...
  },
  {
    ...
    async run(req, res) {
      ...
      try {
        ...
        let results = login.qris_history ? login.qris_history.results || [] : [];
        login = results.filter(e => e && e.status === "IN");
        if (login.length === 0) {
          return res.json({ status: false, error: 'No transaction found' });
        }
